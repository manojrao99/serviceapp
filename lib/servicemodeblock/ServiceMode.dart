...
part 'ServiceModeEvent.dart';
part 'ServiceModeState.dart';
class ServiceMode extends Bloc<ServiceModeEvent, ServiceModeState> {

  ServiceMode() : super(ApiInitial());
  ...
  @override
  Stream<ServiceModeState> mapEventToState(ServiceModeEvent event) async* {
    // Device updatedevice;
    if (!isTimerRunning) {
    ...
    if (event is FetchData) {
      try {
        ...
        // List<Device> filteredNumbers = _deviceList.where((device) =>
      for(int i=0;i<_deviceList.length;i++){
        yield ApiLoading();
        var body=  {
          "deviceID":_deviceList[i].DeviceID,
          "OperationType" :'ServiceMode',
          "DeviceTypeID":_deviceList[i].DeviceTypeID
        };
      try{
        var responce = await ApiHelper().post(path: 'http://aquaf.centralindia.cloudapp.azure.com/servicesF2Fapp/api/farm2fork/common/downlink', postData: body);
           print("responce ${responce}");
     if(responce['success']){

       add(RemoveItemEvent(_deviceList[i]));
         // _deviceList.remove(i);
       // _deviceList[i].lastCommunicated="success";

     }
     else {
       _deviceList[i].lastCommunicated="TTI not Added";
       yield ApiLoaded(posts: _deviceList);
     }
        // print("responce downlink send ${responce}");
        yield ApiLoaded(posts: _deviceList);
      }
          catch(e){}
        }
        ...
        // print("body $devices");
        // final niftyResponse = await ApiHelper().post(
        //     path: postUrl, postData: requestBody);
...
// //             for (int i = 0; i < _deviceList.length; i++) {
// //               print("bool value check ${_deviceList[i].DeviceID ==
// //                   niftyResponse['data'][j]['DeviceID']}");
...
// //                   add(UpdateArrayPositionEvent(i, Device(
// //
// //                       lastCommunicated: niftyResponse['data'][j]['LastDate']
// //                           .toString(),
...
// //                       DeviceID: _deviceList[i].DeviceID,
// //                       hardwareserialnumber: _deviceList[i].hardwareserialnumber,
